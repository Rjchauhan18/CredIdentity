...
def calculate_12_features(input_json_path, output_csv_path):
    ...
    with open(input_json_path, "rb") as infile, open(output_csv_path, "w", newline='') as outfile:
        ...
        for idx, msme in enumerate(items):
            ...
            inflows = sum(t["amount"] for t in aa["transactions"] if t["type"] == "INFLOW" and not t.get("is_bounce_event", False))
            outflows = sum(t["amount"] for t in aa["transactions"] if t["type"] == "OUTFLOW" and not t.get("is_bounce_event", False))
            ...
            total_bounces = sum(1 for t in aa["transactions"] if t["type"] == "OUTFLOW" and t.get("is_bounce_event", False))
            ...
            late_gst_frequency = (
                late_filings / len(gst["filings"])
                if gst["filings"] else 0
            )

            late_gst_frequency += np.random.normal(0,0.08)
            
            late_gst_frequency = np.clip(
                late_gst_frequency,
                0,
                1
            )
            ...
            rev_growth += np.random.normal(0,0.04)
            ...
            emp_trend += np.random.normal(0,0.03)
                
            on_time_epfo = sum(1 for e in epfo["historical_records"] if e["payment_date"] <= e["due_date"])
            epfo_consistency = (
                on_time_epfo /
                len(epfo["historical_records"])
            ) * 100
            
            epfo_consistency += np.random.normal(0,8)
            
            epfo_consistency = np.clip(
                epfo_consistency,
                0,
                100
            )
            ...
            night_txns = sum(
                1
                for t in aa["transactions"]
                if (int(t["timestamp"][11:13]) >= 22)
                or (int(t["timestamp"][11:13]) <= 5)
            )
